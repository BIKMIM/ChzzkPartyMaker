...
function App() {
  ...
  const checkMessage = (role: string, nick: string, msg: string) => {
    if (role === 'streamer' && msg === '파티') {
      setIsShow(true); return;
    }
    if (msg === '취소') {
      setMembers((prev) => prev.filter((m) => !(m.nick === nick && !m.selected))); return;
    }
  ...
  const connectWebSocket = () => {
    if (socketRef.current && socketRef.current.readyState < 2) { return; } // Prevent multiple connections
    ...
    socket.addEventListener('open', () => {
      ...
      if (!chatChannelID) {
          ...
          toast.error("chzzk 채널 ID가 URL에 없습니다!"); return;
      }
      ...
      socket.send(JSON.stringify({ ver: '2', cmd: 10000 })); // Initial ping
    });

    socket.addEventListener('message', (event) => {
      try {
          const data: ChzzkData = JSON.parse(event.data);
          if (data.cmd === 93101 && data.bdy && Array.isArray(data.bdy)) { // Chat messages
          ...
          else if (data.cmd === 0) { // Server ping
              socket.send(JSON.stringify({ ver: '2', cmd: 10000 })); // Send pong
          }
          else if (data.cmd === 10100) { // Connection confirmation
    ...
    socket.addEventListener('error', (error) => {
        ...
        if (socketRef.current === socket) { socketRef.current?.close(); } // Trigger close for potential reconnect
    });
  };
  ...
  const rollupAnimation = (targetMembers: Member[]) => {
    ...
    const interval = setInterval(() => {
        if (targetMembers.length === 0) { clearInterval(interval); setIsRolling(false); toast.error("룰렛 오류: 대상 멤버 없음"); return; }
        const randomIndex = Math.floor(Math.random() * targetMembers.length);
        if (targetMembers[randomIndex]) { setSelectedMember(targetMembers[randomIndex]); }
        else { console.error("Animation Error: Index out of bounds?", randomIndex, targetMembers); clearInterval(interval); setIsRolling(false); toast.error("룰렛 멤버 선택 오류"); return; }
        counter++;

        if (counter > 20) { // End animation
            clearInterval(interval);
            if (targetMembers.length === 0) { setIsRolling(false); toast.error("최종 선택 오류: 대상 멤버 없음"); return; }
            const finalIndex = Math.floor(Math.random() * targetMembers.length);
             if (targetMembers[finalIndex]) {
                ...
                setSelectedMember(selected); // Show final member

                setTimeout(() => { // Update state after animation delay
                  setMembers((prev) => {
                    let updatedMembers = prev.map((m) => m.nick === selected.nick ? { ...m, selected: true } : m);
                    // Auto-exclude logic
                    if (selected.role === '탱커') { updatedMembers = updatedMembers.filter(m => m.selected || m.role !== '탱커'); }
                    else if (selected.role === '힐러') { updatedMembers = updatedMembers.filter(m => m.selected || m.role !== '힐러'); }
                    return updatedMembers;
                  });
                  setIsRolling(false); setSelectedMember(null); // Reset animation state
                }, 1000);
            } else { console.error("Final Selection Error: Index out of bounds?", finalIndex, targetMembers); setIsRolling(false); toast.error("최종 멤버 선택 오류"); }
        }
    }, 100);
  };
  ...
  const handleRandomAll = () => {
    ...
    // Filter available based on needed roles and checkbox state
    ...
    if (!Array.isArray(targetMembers)) { console.error("Filter Error: targetMembers not an array", targetMembers); toast.error("멤버 필터링 오류"); return; }

    if (targetMembers.length === 0) {
      const availableFilteredByCheckboxOnly = available.filter(m => (roleFilter.tank && m.role === '탱커') || (roleFilter.healer && m.role === '힐러') || (roleFilter.dealer && m.role === '딜러') );
      if (availableFilteredByCheckboxOnly.length === 0) { toast.warn('선택한 역할의 대기 멤버가 없습니다!'); }
      else { toast.warn('현재 파티 구성에 맞는 역할의 대기 멤버가 없습니다!'); }
      return;
    }
    rollupAnimation(targetMembers); // Start animation
  };

  // --- WebSocket Effect ---
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get('chzzk')) { connectWebSocket(); } // Connect if channel ID exists
    else { toast.info("URL에 ?chzzk=채널ID 를 추가해주세요."); }
      ...
      {isShow && (
        <div className="w-[950px] h-[800px] bg-black bg-opacity-80 border border-gray-600 flex flex-row rounded-2xl overflow-hidden shadow-2xl">
          ...
          <div className="flex flex-col flex-grow border-r border-gray-700">
            {/* Top: Waiting Members (2/3 height) */}
            ...
            {/* Bottom: Selected Members (1/3 height) */}
            <div className="h-1/3 overflow-y-auto p-4 bg-gray-800 bg-opacity-30 flex items-center">
              <div className="flex flex-row gap-4">
                {members
                  .filter(member => member.selected)
                  .sort((a, b) => { const order = { '탱커': 0, '딜러': 1, '힐러': 2 } as const; return order[a.role] - order[b.role]; }) // Type assertion for safety
                  .map((member, index) => {
                    const classImage = wowClasses.find(c => c.class === member.class)?.image;
                    return (
          ...
          <div className="w-[350px] h-[800px] p-4 overflow-y-auto bg-gray-900 bg-opacity-50 flex flex-col">
            ...
            {/* Class/Spec List (1 column layout) */}
            <div className="space-y-1 mb-4 grid grid-cols-1 gap-1 flex-shrink-0"> {/* Changed to grid-cols-1 */}
              {wowClasses.map((wowClass) => (
                <div key={wowClass.class} className="hover:bg-gray-800 p-1 rounded"> {/* Removed onClick */}
                  <div className="flex items-center gap-1.5">
                    ...
                    <div className="flex flex-row flex-nowrap gap-1"> {/* Ensure no wrapping */}
                      {wowClass.jobs.map((job) => (
                        <span key={job.specificity} className="text-[10px] px-1 py-0.5 rounded border whitespace-nowrap" style={{ borderColor: wowClass.color, color: wowClass.color }}> {/* Added whitespace-nowrap */}
                          {job.specificity}
                        </span>
                      ))}
                    </div>
